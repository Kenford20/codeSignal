...
function companyBotStrategy(trainingData) {
    let correctAnswers = trainingData.filter(task => task[1] === 1);

    return correctAnswers.length === 0 ? 0 :
       correctAnswers.map(task => task[0])
       .reduce((a, b) => a + b) / correctAnswers.length; 
 }
