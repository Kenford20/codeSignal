...
function stolenLunch(note) {
    ...
    for(let i = 0; i < note.length; i++) {
        if(numbers.includes(note[i]))
            decipheredNote += letters[numbers.indexOf(note[i])]
        else if(letters.includes(note[i]))
            decipheredNote += letters.indexOf(note[i]);
        else
            decipheredNote += note[i];
    }
    return decipheredNote;
}
